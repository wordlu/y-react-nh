...
const TreeComponent: React.FC<Props> = ({
  ...
  const onExpandAll = () => {
    const keysArr: any[string] = expandedTreeData?.map(item => item?.key);
  ...
  const onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newExpandedKeys = expandedTreeData
      .map(item => {
  ...
  const retrievalTreeData = useMemo(() => {
    const loop: any = (data: any[]) =>
      data.map(item => {
        const strTitle = item?.title as string;
        const index = strTitle?.indexOf(query);
        const beforeStr = strTitle?.substring(0, index);
        const afterStr = strTitle?.slice(index + query.length);
        const title =
          index > -1 ? (
            <span>
              {beforeStr}
              {query && <span style={{ color: 'red' }}>{query}</span>}
              {afterStr}
            </span>
          ) : (
            <span>{strTitle}</span>
